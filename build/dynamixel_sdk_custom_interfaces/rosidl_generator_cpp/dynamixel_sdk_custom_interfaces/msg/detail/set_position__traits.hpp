  ...
  std::ostream & out)
{
  ...
  {
    if (msg.profile_velocities.size() == 0) {
      out << "profile_velocities: []";
    } else {
      out << "profile_velocities: [";
      size_t pending_items = msg.profile_velocities.size();
      for (auto item : msg.profile_velocities) {
        rosidl_generator_traits::value_to_yaml(item, out);
        if (--pending_items > 0) {
          out << ", ";
        }
  ...
  // member: goal_pwms
  {
    if (msg.goal_pwms.size() == 0) {
      out << "goal_pwms: []";
    } else {
      out << "goal_pwms: [";
      size_t pending_items = msg.goal_pwms.size();
      for (auto item : msg.goal_pwms) {
        rosidl_generator_traits::value_to_yaml(item, out);
        if (--pending_items > 0) {
          out << ", ";
        }
